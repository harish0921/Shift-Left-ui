...
export default class Worker extends BaseCommand {
    ...
    async run(): Promise<void> {
        logger.info('Starting shift left  Worker...')
    ...
    async stopProcess() {
        try {
            ...
            logger.info(`Shutting down shift left  Prediction Worker ${this.predictionWorkerId}...`)
            await predictionWorker.close()

            const upsertWorker = queueManager.getQueue('upsert').getWorker()
            logger.info(`Shutting down shift left  Upsertion Worker ${this.upsertionWorkerId}...`)
            await upsertWorker.close()
        } catch (error) {
            logger.error('There was an error shutting down shift left  Worker...', error)
            await this.failExit()
        }
