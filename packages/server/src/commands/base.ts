...
export abstract class BaseCommand extends Command {
    static flags = {
        // General Settings
        SHIFTLIFT_FILE_SIZE_LIMIT: Flags.string(),
        PORT: Flags.string(),
        ...
        DISABLE_SHIFTLIFT_TELEMETRY: Flags.string(),
        ...
        SHIFTLIFT_SECRETKEY_OVERWRITE: Flags.string(),
        ...
        SHIFTLIFT_EE_LICENSE_KEY: Flags.string(),
    ...
    protected onTerminate() {
        return async () => {
            try {
                ...
                setTimeout(async () => {
                    logger.info('shift left  was forced to shut down after 30 secs')
                    await this.failExit()
                }, 30000)

                await this.stopProcess()
            } catch (error) {
                logger.error('There was an error shutting down shift left ...', error)
            }
        }
    }
