...
export class IdentityManager {
    ...
    private _validateLicenseKey = async () => {
        ...
        const SHIFTLIFT_EE_LICENSE_KEY = process.env.SHIFTLIFT_EE_LICENSE_KEY

        // First check if license key is missing
        if (!SHIFTLIFT_EE_LICENSE_KEY) {
        ...
        try {
            if (process.env.OFFLINE === 'true') {
                const decodedLicense = this._offlineVerifyLicense(SHIFTLIFT_EE_LICENSE_KEY)
                ...
                try {
                    const response = await axios.post(`${LICENSE_URL}/enterprise/verify`, { license: SHIFTLIFT_EE_LICENSE_KEY })
                    ...
                    else throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, GeneralErrorMessage.UNHANDLED_EDGE_CASE)
                } catch (error) {
                    console.error('Error verifying license key:', error)
                    this.licenseValid = false
                    this.currentInstancePlatform = Platform.ENTERPRISE
    ...
    public async updateSubscriptionPlan(req: Request, subscriptionId: string, newPlanId: string, prorationDate: number) {
        ...
        if (!req.user) {
            throw new internalShiftLiftError(StatusCodes.UNAUTHORIZED, GeneralErrorMessage.UNAUTHORIZED)
        }
