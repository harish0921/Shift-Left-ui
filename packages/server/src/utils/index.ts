...
export const REDACTED_CREDENTIAL_VALUE = '_SHIFTLIFT_BLANK_07167752-1a71-43b1-bf8f-4f32252165db'
...
export const getEndingNodes = (
    ...
    let error: internalShiftLiftError | null = null
    for (const endingNode of endingNodes) {
        const endingNodeData = endingNode.data
        if (!endingNodeData) {
            error = new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node ${endingNode.id} data not found`)
        ...
        if (!isEndingNode) {
            if (
                ...
                error = new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending node must be either a Chain or Agent or Engine`)
                continue
            }
        }
    ...
    if (endingNodes.length === 0 || error === null) {
        error = new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Ending nodes not found`)
    }
...
export const getVariableValue = async (
    ...
    while (startIdx < endIdx) {
        ...
        if (substr === '}}' && variableStack.length > 0 && variableStack[variableStack.length - 1].substr === '{{') {
             ...
             * FROM: hello i am ben\n\n\thow are you?
             * TO: hello i am benSHIFTLIFT_NEWLINESHIFTLIFT_NEWLINESHIFTLIFT_TABhow are you?
             */
...
export const getEncryptionKey = async (): Promise<string> => {
    if (process.env.SHIFTLIFT_SECRETKEY_OVERWRITE !== undefined && process.env.SHIFTLIFT_SECRETKEY_OVERWRITE !== '') {
        return process.env.SHIFTLIFT_SECRETKEY_OVERWRITE
    }
    if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
        const secretId = process.env.SECRETKEY_AWS_NAME || 'ShiftLiftEncryptionKey'
        try {
...
export const decryptCredentialData = async (
    ...
    if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
        try {
            if (encryptedData.startsWith('ShiftLiftCredential_')) {
...
export const getOrCreateStoredSecret = async (options: GetOrCreateStoredSecretOptions): Promise<string> => {
    ...
    if (USE_AWS_SECRETS_MANAGER && secretsManagerClient) {
        const prefix = process.env.SECRETKEY_AWS_AUTH_PREFIX || 'ShiftLift'
...
export const _removeCredentialId = (obj: any): any => {
    ...
    for (const [key, value] of Object.entries(obj)) {
        if (key === 'SHIFTLIFT_CREDENTIAL_ID') continue
