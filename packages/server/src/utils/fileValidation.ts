 ...
 * @throws {internalShiftLiftError} If validation fails, throws BAD_REQUEST error
 * @example
 * ```typescript
...
export function validateFileMimeTypeAndExtensionMatch(filename: string, mimetype: string): void {
    try {
        ...
        throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, getErrorMessage(error))
    }
}
