...
export const createFileAttachment = async (req: Request) => {
    ...
    if (!chatflowid || !isValidUUID(chatflowid)) {
        throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, 'Invalid chatflowId format - must be a valid UUID')
    }
    if (isPathTraversal(chatflowid) || (chatId && isPathTraversal(chatId))) {
        throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, 'Invalid path characters detected')
    }
    ...
    if (!fileUploadEnabled) {
        throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, 'File upload is not enabled for this chatflow')
    }
