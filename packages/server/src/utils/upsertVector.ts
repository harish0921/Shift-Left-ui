...
export const executeUpsert = async ({
    ...
    if (vsNodesWithFileUpload.length > 1) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, 'Multiple vector store nodes with fileUpload enabled')
    ...
    if (vsNodes.length > 1 && !stopNodeId) {
        ...
        throw new internalShiftLiftError(StatusCodes.NOT_FOUND, 'No vector node found')
    }
...
export const upsertVector = async (req: Request, isInternal: boolean = false) => {
    ...
    try {
        ...
        if (!chatflow) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowid} not found`)
        }
        ...
        if (!isInternal) {
            ...
            if (!isKeyValidated) {
                throw new internalShiftLiftError(StatusCodes.UNAUTHORIZED, `Unauthorized`)
            }
        }
        ...
        if (!workspace) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Workspace ${chatflowWorkspaceId} not found`)
        }
        const workspaceId = workspace.id

        if (workspaceId !== req.user?.activeWorkspaceId) throw new internalShiftLiftError(StatusCodes.UNAUTHORIZED, 'Unauthorized')

        const org = await appServer.AppDataSource.getRepository(Organization).findOneBy({
            id: workspace.organizationId
        })
        if (!org) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Organization ${workspace.organizationId} not found`)
        }
        ...
        if (process.env.MODE === MODE.QUEUE) {
            ...
            appServer.metricsProvider?.incrementCounter(SHIFTLIFT_METRIC_COUNTERS.VECTORSTORE_UPSERT, {
        ...
        appServer.metricsProvider?.incrementCounter(SHIFTLIFT_METRIC_COUNTERS.VECTORSTORE_UPSERT, { status: SHIFTLIFT_COUNTER_STATUS.FAILURE })

        if (e instanceof internalShiftLiftError && e.statusCode === StatusCodes.UNAUTHORIZED) {
            throw e
        } else {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, getErrorMessage(e))
        }
    }
}
