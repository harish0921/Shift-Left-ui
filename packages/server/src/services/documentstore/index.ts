...
const deleteLoaderFromDocumentStore = async (
    ...
    try {
        ...
        if (found) {
            ...
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Unable to locate loader in Document Store ${entity.name}`)
        }
    } catch (error) {
        throw new internalShiftLiftError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getDocumentStoreFileChunks = async (
    ...
    try {
        ...
        if (!chunksWithCount) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Chunks with docId: ${docId} not found`)
        }
...
const deleteDocumentStoreFileChunk = async (storeId: string, docId: string, chunkId: string, workspaceId: string) => {
    try {
        ...
        if (!found) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Document store loader ${docId} not found`)
        }
        ...
        if (!tbdChunk) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Document Chunk ${chunkId} not found`)
        }
...
const deleteVectorStoreFromStore = async (storeId: string, workspaceId: string, docId?: string) => {
    try {
        ...
        if (!entity.embeddingConfig) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Embedding for Document store ${storeId} not found`)
        }

        if (!entity.vectorStoreConfig) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Vector Store for Document store ${storeId} not found`)
        }
...
const _splitIntoChunks = async (
    ...
    try {
        ...
        // doc loader configs
        const nodeData = {
            credential: data.credential || data.loaderConfig['SHIFTLIFT_CREDENTIAL_ID'] || undefined,
...
const queryVectorStore = async (data: ICommonObject) => {
    try {
        ...
        if (!entity) {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Document store ${data.storeId} not found`)
        }
        const options: ICommonObject = {
            chatflowid: uuidv4(),
            appDataSource: appServer.AppDataSource,
        ...
        if (!entity.embeddingConfig) {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding for ${data.storeId} is not configured`)
        }

        if (!entity.vectorStoreConfig) {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector Store for ${data.storeId} is not configured`)
        }
        ...
        if (!retriever) {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create retriever`)
        }
        const startMillis = Date.now()
        const results = await retriever.invoke(data.query, undefined)
        if (!results) {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to retrieve results`)
        }
...
const _createEmbeddingsObject = async (
    ...
    if (!embeddingObj) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create EmbeddingObj`)
    }
    return embeddingObj
}
...
const _createRecordManagerObject = async (
    ...
    if (!recordManagerObj) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Failed to create RecordManager obj`)
    }
...
const upsertDocStore = async (
    ...
    if (data.metadata) {
        try {
            ...
            throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, `Error: Invalid metadata`)
        }
    }
    ...
    if (docId) {
        ...
        if (!loader) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Document loader ${docId} not found`)
        }
    ...
    if (!loaderName || !loaderId || !loaderConfig) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Loader not configured`)
    }

    if (!vectorStoreName || !vectorStoreConfig) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Vector store not configured`)
    }

    if (!embeddingName || !embeddingConfig) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Embedding not configured`)
    }
...
const generateDocStoreToolDesc = async (docStoreId: string, selectedChatModel: ICommonObject): Promise<ICommonObject> => {
    try {
        ...
        if (!chunks?.length) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `DocumentStore ${docStoreId} chunks not found`)
        }
        ...
        if (selectedChatModel && Object.keys(selectedChatModel).length > 0) {
            ...
            const newNodeInstance = new nodeModule.nodeClass()
            const nodeData = {
                credential: selectedChatModel.credential || selectedChatModel.inputs['SHIFTLIFT_CREDENTIAL_ID'] || undefined,
