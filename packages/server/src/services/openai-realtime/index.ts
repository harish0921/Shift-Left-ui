...
import { ICommonObject, INodeData } from 'shiftleftai-components'
...
const buildAndInitTool = async (chatflowid: string, _chatId?: string, _apiMessageId?: string) => {
    ...
    if (!toolAgentNode) {
        throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Agent with tools not found in chatflow ${chatflowid}`)
    }
...
const executeAgentTool = async (
    ...
    try {
        ...
        if (!tool) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Tool ${toolName} not found`)
        }
