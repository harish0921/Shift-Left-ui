...
const createTool = async (requestBody: any, orgId: string): Promise<any> => {
    try {
        ...
        appServer.metricsProvider?.incrementCounter(SHIFTLIFT_METRIC_COUNTERS.TOOL_CREATED, { status: SHIFTLIFT_COUNTER_STATUS.SUCCESS })
        return dbResponse
    } catch (error) {
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.createTool - ${getErrorMessage(error)}`)
    }
}
...
const getAllTools = async (workspaceId?: string, page: number = -1, limit: number = -1) => {
    try {
        ...
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.getAllTools - ${getErrorMessage(error)}`)
    }
}
...
const getToolById = async (toolId: string, workspaceId: string): Promise<any> => {
    try {
        ...
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.getToolById - ${getErrorMessage(error)}`)
    }
}
...
const updateTool = async (toolId: string, toolBody: any, workspaceId: string): Promise<any> => {
    try {
        ...
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.updateTool - ${getErrorMessage(error)}`)
    }
}
    ...
    try {
        for (const data of newTools) {
            if (data.id && !validate(data.id)) {
                throw new internalShiftLiftError(StatusCodes.PRECONDITION_FAILED, `Error: importTools - invalid id!`)
            }
        }
        ...
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: toolsService.importTools - ${getErrorMessage(error)}`)
    }
}
