...
import { SHIFTLIFT_COUNTER_STATUS, SHIFTLIFT_METRIC_COUNTERS } from '../../Interface.Metrics'
...
const createAssistant = async (requestBody: any, orgId: string): Promise<Assistant> => {
    try {
        ...
        if (!requestBody.details) {
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Invalid request body`)
        }
        ...
        if (requestBody.type === 'CUSTOM') {
            ...
            appServer.metricsProvider?.incrementCounter(SHIFTLIFT_METRIC_COUNTERS.ASSISTANT_CREATED, {
                status: SHIFTLIFT_COUNTER_STATUS.SUCCESS
            })
            return dbResponse
        }
        ...
        try {
            ...
            if (!credential) {
                throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Credential ${requestBody.credential} not found`)
            }
            ...
            if (!openAIApiKey) {
                throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `OpenAI ApiKey not found`)
            }
            ...
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error creating new assistant - ${getErrorMessage(error)}`)
        }
        const newAssistant = new Assistant()
        Object.assign(newAssistant, requestBody)
        ...
        appServer.metricsProvider?.incrementCounter(SHIFTLIFT_METRIC_COUNTERS.ASSISTANT_CREATED, { status: SHIFTLIFT_COUNTER_STATUS.SUCCESS })
...
const deleteAssistant = async (assistantId: string, isDeleteBoth: any, workspaceId?: string): Promise<DeleteResult> => {
    try {
        ...
        try {
            ...
            if (!credential) {
                throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Credential ${assistant.credential} not found`)
            }
            ...
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error deleting assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new internalShiftLiftError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const updateAssistant = async (assistantId: string, requestBody: any, workspaceId?: string): Promise<Assistant> => {
    try {
        ...
        try {
            ...
            if (!credential) {
                throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Credential ${body.credential} not found`)
            }
            ...
            throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error updating assistant - ${getErrorMessage(error)}`)
        }
    } catch (error) {
        throw new internalShiftLiftError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const getTools = async (): Promise<any> => {
    try {
        ...
        throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error: assistantsService.getTools - ${getErrorMessage(error)}`)
    }
}
