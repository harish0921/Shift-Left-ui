import { ICommonObject, removeFolderFromStorage } from 'shiftlift-components'
...
export function validateChatflowType(type: ChatflowType | undefined) {
    if (!Object.values(EnumChatflowType).includes(type as EnumChatflowType))
        throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, ChatflowErrorMessage.INVALID_CHATFLOW_TYPE)
}
...
const getChatflowByApiKey = async (apiKeyId: string, workspaceId?: string, keyonly?: unknown): Promise<any> => {
    try {
        ...
        if (dbResponse.length < 1) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Chatflow not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        throw new internalShiftLiftError(
...
const getChatflowById = async (chatflowId: string, workspaceId?: string): Promise<any> => {
    try {
        if (!isValidUUID(chatflowId)) {
            throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, ChatflowErrorMessage.INVALID_CHATFLOW_ID)
        }
        const appServer = getRunningExpressApp()
        ...
        if (!dbResponse) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Chatflow ${chatflowId} not found in the database!`)
        }
        return dbResponse
    } catch (error) {
        if (error instanceof internalShiftLiftError) {
            throw error
        }
        throw new internalShiftLiftError(
            StatusCodes.INTERNAL_SERVER_ERROR,
...
const saveChatflow = async (
    ...
    appServer.metricsProvider?.incrementCounter(
        dbResponse?.type === 'MULTIAGENT' ? SHIFTLIFT_METRIC_COUNTERS.AGENTFLOW_CREATED : SHIFTLIFT_METRIC_COUNTERS.CHATFLOW_CREATED,
...
const updateChatflow = async (
    ...
    if (updateChatFlow.chatbotConfig) {
        try {
            ...
            throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, `Invalid chatbotConfig: ${message}`)
        }
    }
    ...
    try {
        ...
        if (dbResponse.chatbotConfig || uploadsConfig) {
            try {
                ...
                throw new internalShiftLiftError(StatusCodes.INTERNAL_SERVER_ERROR, `Error parsing Chatbot Config for Chatflow ${chatflowId}`)
            }
        }
        return 'OK'
    } catch (error) {
