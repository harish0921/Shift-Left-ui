...
export class Prometheus implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        const serviceName: string = process.env.METRICS_SERVICE_NAME || 'shift left '
        ...
        try {
            const versionGaugeCounter = new promClient.Gauge({
                name: 'shiftlift_version_info',
                help: 'shift left  version info.',
                labelNames: ['version'],
                registers: [this.register] // Explicitly set the registry
            })
            ...
            this.counters.set('shiftlift_version', versionGaugeCounter)
        } catch (error) {
            // If metric already exists, get it from the registry
            const existingMetric = this.register.getSingleMetric('shiftlift_version')
            if (existingMetric) {
                this.counters.set('shiftlift_version', existingMetric as promClient.Gauge<string>)
            }
        }
    ...
    public incrementCounter(counter: SHIFTLIFT_METRIC_COUNTERS, payload: any) {
    ...
    private registerMetrics() {
        if (process.env.METRICS_INCLUDE_NODE_METRICS !== 'false') {
            ...
            promClient.collectDefaultMetrics({
                ...
                prefix: 'shiftlift_' // Add a prefix to avoid conflicts
            })
        }
