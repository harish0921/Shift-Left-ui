...
export class OpenTelemetry implements IMetricsProvider {
    ...
    async initializeCounters(): Promise<void> {
        try {
            ...
            const shiftliftVersion = await getVersion()

            this.resource = new Resource({
                [ATTR_SERVICE_NAME]: process.env.METRICS_SERVICE_NAME || 'shift left ',
                [ATTR_SERVICE_VERSION]: shiftliftVersion.version // Version as a label
            })
            ...
            try {
                ...
                if (!createdMetrics.has('shiftlift_version')) {
                    const versionGuage = meter.createGauge('shiftlift_version', {
                        description: 'shift left  version'
                    })
                    // remove the last dot from the version string, e.g. 2.1.3 -> 2.13 (gauge needs a number - float)
                    const formattedVersion = shiftliftVersion.version.replace(/\.(\d+)$/, '$1')
                    versionGuage.record(parseFloat(formattedVersion))
                    createdMetrics.set('shiftlift_version', true)
                }
            } catch (error) {
