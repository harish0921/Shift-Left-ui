...
export class AccountService {
    ...
    public async resendVerificationEmail({ email }: { email: string }) {
        ...
        try {
            ...
            if (!user) throw new internalShiftLiftError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            if (user && user.status === UserStatus.ACTIVE)
                throw new internalShiftLiftError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_EMAIL_ALREADY_EXISTS)

            if (!user.email) throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
    ...
    private async ensureOneOrganizationOnly(queryRunner: QueryRunner) {
        ...
        if (organizations.length > 0) throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, 'You can only have one organization')
    }
    ...
    private async createRegisterAccount(data: AccountDTO, queryRunner: QueryRunner) {
        ...
        switch (platform) {
            ...
            case Platform.CLOUD: {
                ...
                if (!data.user.email) throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
            ...
            case Platform.ENTERPRISE: {
                if (data.user.tempToken) {
                    ...
                    if (user.email.toLowerCase() !== data.user.email?.toLowerCase())
                        throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_EMAIL)
                    ...
                    if (!organizationUser)
                        throw new internalShiftLiftError(StatusCodes.NOT_FOUND, OrganizationUserErrorMessage.ORGANIZATION_USER_NOT_FOUND)
                    ...
                    if (!assignedOrganization)
                        throw new internalShiftLiftError(StatusCodes.NOT_FOUND, OrganizationErrorMessage.ORGANIZATION_NOT_FOUND)
                    data.organization = assignedOrganization
                    const tokenExpiry = new Date(user.tokenExpiry!)
                    const today = new Date()
                    if (today > tokenExpiry) throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.EXPIRED_TEMP_TOKEN)
                    data.user.tempToken = ''
                    data.user.tokenExpiry = null
                    data.user.name = name
                    data.user.status = UserStatus.ACTIVE
            ...
            default:
                throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, GeneralErrorMessage.UNHANDLED_EDGE_CASE)
        }
    ...
    private async saveInviteAccount(data: AccountDTO, currentUser?: Express.User) {
        ...
        try {
            ...
            if (!workspace) throw new internalShiftLiftError(StatusCodes.NOT_FOUND, WorkspaceErrorMessage.WORKSPACE_NOT_FOUND)
            ...
            if (!role) throw new internalShiftLiftError(StatusCodes.NOT_FOUND, RoleErrorMessage.ROLE_NOT_FOUND)
            data.role = role
            const user = await this.userService.readUserByEmail(data.user.email, queryRunner)
            if (!user) {
    ...
    public async login(data: AccountDTO) {
        ...
        try {
            if (!data.user.credential) {
                ...
                throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_USER_CREDENTIAL)
            }
            const user = await this.userService.readUserByEmail(data.user.email, queryRunner)
            if (!user) {
                await auditService.recordLoginActivity(data.user.email || '', LoginActivityCode.UNKNOWN_USER, 'Login Failed')
                throw new internalShiftLiftError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            }
            if (!user.credential) {
            ...
            if (!compareHash(data.user.credential, user.credential)) {
                await auditService.recordLoginActivity(user.email || '', LoginActivityCode.INCORRECT_CREDENTIAL, 'Login Failed')
                throw new internalShiftLiftError(StatusCodes.UNAUTHORIZED, UserErrorMessage.INCORRECT_USER_EMAIL_OR_CREDENTIALS)
            }
            ...
            if (user.status === UserStatus.UNVERIFIED) {
                ...
                throw new internalShiftLiftError(StatusCodes.UNAUTHORIZED, UserErrorMessage.USER_EMAIL_UNVERIFIED)
            }
            ...
            if (Array.isArray(wsUserOrUsers)) {
                if (wsUserOrUsers.length > 0) {
                    ...
                    throw new internalShiftLiftError(StatusCodes.NOT_FOUND, WorkspaceUserErrorMessage.WORKSPACE_USER_NOT_FOUND)
                }
            }
    ...
    public async verify(data: AccountDTO) {
        ...
        try {
            await queryRunner.startTransaction()
            if (!data.user.tempToken) throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_TEMP_TOKEN)
            const user = await this.userService.readUserByToken(data.user.tempToken, queryRunner)
            if (!user) throw new internalShiftLiftError(StatusCodes.NOT_FOUND, UserErrorMessage.USER_NOT_FOUND)
            data.user = user
    ...
    public async resetPassword(data: AccountDTO) {
        ...
        try {
            ...
            if (!user.tempToken || user.tempToken !== data.user.tempToken)
                throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_TEMP_TOKEN)

            const tokenExpiry = user.tokenExpiry
            if (!tokenExpiry) throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.INVALID_TEMP_TOKEN)

            const tokenExpiryMoment = moment(tokenExpiry)
            if (!tokenExpiryMoment.isValid() || moment().isAfter(tokenExpiryMoment))
                throw new internalShiftLiftError(StatusCodes.BAD_REQUEST, UserErrorMessage.EXPIRED_TEMP_TOKEN)
