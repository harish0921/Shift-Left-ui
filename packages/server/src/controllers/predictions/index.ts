...
const createPrediction = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        if (!chatflow) {
            throw new internalShiftLiftError(StatusCodes.NOT_FOUND, `Chatflow ${req.params.id} not found`)
        }
        ...
        if (isDomainAllowed) {
            ...
            throw new internalShiftLiftError(StatusCodes.FORBIDDEN, unauthorizedOriginError)
        }
    } catch (error) {
        next(error)
    }
